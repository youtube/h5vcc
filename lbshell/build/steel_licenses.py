...
def FindThirdPartyDependencies():
  ...
  for path, dirs, files in os.walk(build_dir):
    for dep in fnmatch.filter(files, "*.d"):
      ...
      depText = f.read()
      matches = third_party_re.findall(depText)
      for m in matches:
...
def CreateSteelPruneList(third_party_dirs):
  ...
  if len(third_party_deps) == 0:
    raise licenses.LicenseError('No dependencies found. Check the build directory.')
...
def GenerateLicense(output_filename = None):
  """Generate list of licenses in html form, dumping the result to output file."""
...
def ScanDirectories():
  ...
  third_party_dirs = set(licenses.FindThirdPartyDirs(licenses.PRUNE_PATHS, os.getcwd()))
  prune_paths = CreateSteelPruneList(third_party_dirs)
