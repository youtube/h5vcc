...
#if defined(HAVE_PTW32_CONFIG_H) || defined(__LB_SHELL__)
#include "config.h"
...
// For compatibility with calling code, use a scalar type for pthread_t.
// We pack the contents of a ptw32_handle_t into a pthread_t.
// NOTE: This assumes void* and int are both 32 bits.
...
#if defined(_MSC_VER) && defined(__LB_SHELL__)
/* Disable MSVC warning about returning a UDT (user-defined type) from a
...
/*
 * Get attributes pertaining to the given thread.
 * dest_attr must have been initialized with pthread_attr_init() and should be
 * destroyed by caller with pthread_attr_destroy().
 * Return 0 on success.
 */
PTW32_DLLPORT int PTW32_CDECL pthread_getattr_np(pthread_t thread, pthread_attr_t* dest_attr);
...
// LB_SHELL: pthread_tests expects PTHREAD_MAX_THREADS to be defined.
