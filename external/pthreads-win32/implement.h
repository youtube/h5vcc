 ...
 */
#if defined(__LB_SHELL__) && defined(HAVE_WINSOCK_H)
#include <winsock.h>
#endif
...
#if defined(__LB_SHELL__)
static INLINE ptw32_handle_t to_internal(pthread_t handle) {
  ptw32_handle_t internal;
  internal.p = (void*)((handle >> 32) & 0xffffffff);
  internal.x = (handle & 0xffffffff);
  return internal;
}

static INLINE pthread_t to_pthread(ptw32_handle_t internal) {
  unsigned int high_bits = (unsigned int)internal.p;
  return ((unsigned long long)(high_bits) << 32) | internal.x;
}
#else
#define to_pthread(x) x
#define to_internal(x) x
#endif
...
struct ptw32_thread_t_
{
  ...
  ptw32_handle_t ptHandle;		/* This thread's permanent pthread_t handle */
 ...
 */
  ...
  void ptw32_robust_mutex_add(pthread_mutex_t* mutex, ptw32_handle_t self);
  ...
  void ptw32_threadDestroy (ptw32_handle_t tid);

  void ptw32_pop_cleanup_all (int execute);

  ptw32_handle_t ptw32_new (void);

  ptw32_handle_t ptw32_threadReusePop (void);

  void ptw32_threadReusePush (ptw32_handle_t thread);

  int ptw32_getprocessors (int *count);

  int ptw32_setthreadpriority (ptw32_handle_t thread, int policy, int priority);
  ...
  void ptw32_callUserDestroyRoutines (ptw32_handle_t thread);
