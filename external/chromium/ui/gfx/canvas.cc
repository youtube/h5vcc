...
#if defined (__LB_SHELL__)
    gfx::Size size(image_rep.pixel_width(), image_rep.pixel_height());
    canvas_capacity_ = gfx::ToFlooredSize(
      gfx::ScaleSize(size, ui::GetScaleFactorScale(scale_factor)));
#endif
}
...
void Canvas::RecreateBackingCanvas(const gfx::Size& size,
  ...
  gfx::Size pixel_size = gfx::ToFlooredSize(
      gfx::ScaleSize(size, ui::GetScaleFactorScale(scale_factor)));
#if defined(__LB_SHELL__)
    // Only allow resizing to a smaller size.
    if (pixel_size.x() <= canvas_capacity_.x() &&
        pixel_size.y() <= canvas_capacity_.y()) {
      owned_canvas_->getDevice()->resizeBitmap(pixel_size.width(),
                                               pixel_size.height());
    }
#else
  owned_canvas_ = skia::AdoptRef(skia::CreatePlatformCanvas(pixel_size.width(),
