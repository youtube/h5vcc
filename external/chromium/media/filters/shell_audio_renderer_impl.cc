...
void ShellAudioRendererImpl::OnDecoderSelected(
  ...
  // TODO(xiaomings) : Find a proper way to check native channel support here,
  ...
  if (status == AudioDecoder::kOk) {
    if (!buffer) {
      ...
      if (buffer->IsEndOfStream()) {
        ...
        // TODO(xiaomings) : Change this to DCHECK_LE once we support Read with
        //                   arbitrary size.
    ...
    if (decoded_audio_bus_ != NULL && decode_complete_) {
      ...
      if (decode_status_ != AudioDecoder::kOk) {
        ...
        if (end_of_stream_state_ == kRenderedEOS) {
          ...
          // TODO(xiaomings) : Change this to DCHECK_LE and fill only
