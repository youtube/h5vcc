...
class MEDIA_EXPORT MediaCodecBridge {
 public:
  ...
  // Fill the content of the output buffer into the given buffer. This call
  // must be called after DequeueOutputBuffer() and before ReleaseOutputBuffer.
  // The size of the buffer pointed by 'buffer_to_fill' is at least 'size'
  // bytes.
  void FillBufferWithOutputBuffer(int index, size_t offset, size_t size,
                                  uint8* buffer_to_fill);
