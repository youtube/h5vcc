...
void FrameView::paintContents(GraphicsContext* p, const IntRect& rect)
{
    ...
    // This assert fails during a game pause with very very specific timing.
    // Since there's already a check for needsLayout() right after the assert,
    // it's harmless to omit the assertion.  Omitting the assertion makes
    // life much much easier during debugging.
    //ASSERT(!needsLayout());
    if (needsLayout())
        return;
