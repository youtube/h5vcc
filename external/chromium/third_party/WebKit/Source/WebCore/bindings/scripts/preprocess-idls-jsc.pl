...
sub injectNamespace
{
    ...
    print FH "    [CustomGetter] readonly attribute $type $ns_top;\n";
    print FH "};\n";
    close FH;
...
sub generateNamespaceCpp
{
    ...
    mkdir($cppPath);
    my $filename = "$cppPath/JSDOMWindow__inject__$ns_top.cpp";
    my $property = lc($ns_top);

    open FH, "> $filename" or die "Cannot open $filename for writing!\n";
    print FH "#include \"config.h\"\n";
    print FH "#include \"JSDOMWindowCustom.h\"\n";

    foreach my $ns_info (@ns_list) {
        my $final_component = $ns_info->{"final_component"};
        print FH "#include \"JS${final_component}.h\"\n";
    }
    print FH "\n";

    print FH "using namespace JSC;\n";
    print FH "namespace WebCore {\n";
    print FH "\n";

    print FH "JSValue JSDOMWindow::$property(JSC::ExecState *exec) const {\n";
    print FH "  static Weak<JSObject> cached_$property;\n";
    print FH "  JSObject *temp_$property = cached_$property.get();\n";

    print FH "  if (!temp_$property) {\n";
    print FH "    temp_$property = constructEmptyObject(exec);\n";
    print FH "    cached_$property = PassWeak<JSObject>(temp_$property);\n";
    ...
    print FH "  return temp_$property;\n";
    print FH "}\n";
    print FH "\n";
...
sub generateNSAttachment
{
    ...
    for (my $i = 1; $i < scalar(@ns); $i++) {
        ...
        if (!$attached->{$ns_path}) {
          my $id = "Identifier(exec, \"$component\")";
          print $fh "\n";
          print $fh "    // Attach $ns_path\n";
          print $fh "    JSObject *$ns_path_name = constructEmptyObject(exec);\n";
          print $fh "    $parent_name\->putDirect(exec->globalData(), $id, $ns_path_name);\n";
          $attached->{$ns_path} = 1;
        }
    ...
    if ($constructor) {
        my $id = "Identifier(exec, \"$final_component\")";
        my $value = "getDOMConstructor<JS${final_component}Constructor>(exec, this)";
        print $fh "    // Attach $ns_path\n";
        print $fh "    $parent_name\->putDirect(exec->globalData(), $id, $value);\n";
    } else {
        print $fh "    // Attach $ns_path methods\n";
        foreach my $func (@functions) {
            my $name = $func->signature->name;
            my $id = "Identifier(exec, \"$name\")";
            my $uc_name = uc(substr($name, 0, 1)) . substr($name, 1);
            my $native_func = "js${final_component}ConstructorFunction${uc_name}";
            my $value = "JSFunction::create(exec, exec->lexicalGlobalObject(), 0, \"$name\", $native_func)";
            print $fh "    $parent_name\->putDirect(exec->globalData(), $id, $value);\n";
        }
        ...
        foreach my $attr (@attributes) {
            ...
            my $id = "Identifier(exec, \"$name\")";
            my $uc_name = uc(substr($name, 0, 1)) . substr($name, 1);
            my $value = "js${final_component}Constructor${uc_name}(exec, JSValue(), Identifier())";
            print $fh "    $parent_name\->putDirect(exec->globalData(), $id, $value);\n";
        }
    }
