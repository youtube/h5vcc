...
static JSGlobalData* s_globalData = NULL;
...
void JSDOMWindowBase::clearCommonJSGlobalData()
{
    ASSERT(isMainThread());
    if (s_globalData) {
        s_globalData->deref();
...
JSGlobalData* JSDOMWindowBase::commonJSGlobalData()
{
    ...
    if (!s_globalData) {
        ...
        s_globalData = JSGlobalData::createLeaked(LargeHeap).leakRef();
        s_globalData->timeoutChecker.setTimeoutInterval(10000); // 10 seconds
#ifndef NDEBUG
        s_globalData->exclusiveThread = currentThread();
#endif
        initNormalWorldClientData(s_globalData);
    }
