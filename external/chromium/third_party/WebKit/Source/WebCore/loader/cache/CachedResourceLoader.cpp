...
#if defined(__LB_SHELL__)
    // If we let them, these URLs in m_validatedURLs accumulate in memory over
    // the lifetime of the app and are never released.  This structure is only
    // useful in the following scenario:
    //
    // A page is loading, and has not finished loading yet.  That page, in it's
    // original, non-DOM-manipulated form, contains at least two references to
    // the same resource URL.  If that is the case, the second and subsequent
    // references will not result in further requests to load that resource.
    //
    // As this would be a negligibly small win for YouTube, and the expense is
    // never-ending memory consumption, this really must be disabled.
#else
