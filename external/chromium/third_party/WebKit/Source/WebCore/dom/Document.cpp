...
#elif defined(__LB_SHELL__)
    // Fake SVG support!  Creates a dummy HTMLElement.
    // There is already a fallback below which creates an Element object, but
    // that isn't high-level enough (HTMLElement > StyledElement > Element)
    // for JavaScript to be able to locate it in the DOM by its class.
    // So this compromise keeps certain scripts from throwing exceptions when
    //.they have the audacity to assume that they will find <svg> elements
    // that absolutely do exist in the HTML.
    else if (qName.namespaceURI() == SVGNames::svgNamespaceURI)
        e = HTMLElement::create(qName, this);
#endif
#if ENABLE(MATHML)
