...
#if defined(__LB_SHELL__)
static const char interactivePrompt[] = "JSC>\n";
#else
...
#if defined(__LB_SHELL__)
extern void jscoutput(const char*);
extern char* getandclearjscoutput();
#endif
...
#else
        jscoutput(exec->argument(i).toString(exec)->value(exec).utf8().data());
#endif
    }
...
static bool runWithScripts(GlobalObject* globalObject, const Vector<Script>& scripts, bool dump)
{
    ...
    for (size_t i = 0; i < scripts.size(); i++) {
        ...
        char *exception = NULL;
        if (evaluationException) {
            exception = strdup(evaluationException.toString(globalObject->globalExec())->value(globalObject->globalExec()).utf8().data());
        }

#if defined(__LB_SHELL__)
        if (evaluationException) {
          if (isTerminatedExecutionException(evaluationException)) {
            success = true;
          } else {
            jscoutput("Exception: ");
            jscoutput(exception);
...
#endif
#if defined(__LB_SHELL__)
        char *output(getandclearjscoutput());
        printf("%s", output);
        free(output);
#endif
        if (evaluationException)
