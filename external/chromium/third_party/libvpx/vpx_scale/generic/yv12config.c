...
#else
    {
      uint8_t *y_buffer_unaligned = ybf->buffer_alloc +
          (border * y_stride) + border;
      uint8_t *u_buffer_unaligned = ybf->buffer_alloc + yplane_size +
          (uv_border_h * uv_stride) + uv_border_w;
      uint8_t *v_buffer_unaligned = ybf->buffer_alloc + yplane_size +
          uvplane_size + (uv_border_h * uv_stride) + uv_border_w;

      size_t y_align_increase =
          ((size_t)(y_buffer_unaligned + 255) & (size_t)~255) -
          (size_t)y_buffer_unaligned;
      size_t u_align_increase =
          ((size_t)(u_buffer_unaligned + 255) & (size_t)~255) -
          (size_t)u_buffer_unaligned;
      size_t v_align_increase =
          ((size_t)(v_buffer_unaligned + 255) & (size_t)~255) -
          (size_t)v_buffer_unaligned;

      ybf->y_buffer = y_buffer_unaligned + y_align_increase;
      ybf->u_buffer = u_buffer_unaligned + u_align_increase;
      ybf->v_buffer = v_buffer_unaligned + v_align_increase;
    }
#endif  // CONFIG_VP9_HIGHBITDEPTH
