...
void SkARGB32_Shader_Blitter::blitRect(int x, int y, int width, int height) {
    ...
    if (fConstInY) {
        if (fShadeDirectlyIntoDevice) {
            ...
            // SkColorShader simply sets the color across the entire region.
            // In this case, use sk_memset32, which can have a much faster
            // platform specific implementation
            SkColor color;
            SkShader::GradientInfo info;
            info.fColors = &color;
            info.fColorCount = 1;
            SkShader::GradientType type = fShader->asAGradient(&info);
            if (type == SkShader::kColor_GradientType) {
              if (width == fDevice.width()) {
                sk_memset32(device, info.fColors[0], width*height);
              } else {
                do {
                  sk_memset32(device, info.fColors[0], width);
