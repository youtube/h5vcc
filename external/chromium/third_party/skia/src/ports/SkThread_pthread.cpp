...
#if defined (__LB_SHELL__)
// For LB_SHELL, always use our own even on Linux just to keep
// the platforms the same.
#undef GCC_INTRINSIC
#endif
...
int32_t sk_atomic_inc(int32_t* addr)
{
    return LB::Platform::atomic_inc_32(reinterpret_cast<uint32_t*>(addr)) - 1;
}

int32_t sk_atomic_add(int32_t* addr, int32_t inc)
{
    return LB::Platform::atomic_add_32(
        reinterpret_cast<uint32_t*>(addr), inc) - inc;
}

int32_t sk_atomic_dec(int32_t* addr)
{
    return LB::Platform::atomic_dec_32(reinterpret_cast<uint32_t*>(addr)) + 1;
}
...
int32_t sk_atomic_conditional_inc(int32_t* addr)
{
    return LB::Platform::atomic_conditional_inc(
