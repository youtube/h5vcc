...
#if defined(__LB_SHELL__)
// This is the reusable object buffer
static std::vector<void*> s_tile_quad_reuse_buffer;
void TileDrawQuad::operator delete(void* p) {
  s_tile_quad_reuse_buffer.push_back(p);
}
#endif

scoped_ptr<TileDrawQuad> TileDrawQuad::Create() {
#if defined(__LB_SHELL__)
    if (s_tile_quad_reuse_buffer.size() > 0) {
      void* block = s_tile_quad_reuse_buffer.back();
      s_tile_quad_reuse_buffer.pop_back();
      return make_scoped_ptr(new (block) TileDrawQuad);
    }
#endif
