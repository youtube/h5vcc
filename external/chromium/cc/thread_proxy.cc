...
void ThreadProxy::RunOnCCThread(const base::Closure& func)
{
    DCHECK(isMainThread());
    TRACE_EVENT0("cc", "ThreadProxy::RunOnCCThread");
    Proxy::implThread()->postTask(
        base::Bind(&ThreadProxy::RunOnCCThreadOnImplThread,
                   base::Unretained(this),
                   func));
}
#endif
...
#if defined(__LB_SHELL__)
void ThreadProxy::RunOnCCThreadOnImplThread(const base::Closure& func)
{
    DCHECK(isImplThread());
    TRACE_EVENT0("cc", "ThreadProxy::RunOnCCThreadOnImplThread");
    func.Run();
}
#endif
...
#if defined(__LB_SHELL__)
void ThreadProxy::ensureContextCurrent()
{
    TRACE_EVENT0("cc", "ensureContextCurrent");
    DCHECK(isImplThread());
    m_layerTreeHostImpl->outputSurface()->Context3D()->makeContextCurrent();
}
#endif
...
#if defined(__LB_SHELL__)
    m_outputSurfaceBeforeInitializationOnImplThread->Context3D()->makeContextCurrent();
#endif
