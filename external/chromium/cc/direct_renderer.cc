...
public:
    static RenderPassReuseList* get_instance();
};

static RenderPassReuseList s_render_pass_reuse_list;
RenderPassReuseList* RenderPassReuseList::get_instance() {
    return &s_render_pass_reuse_list;
}
#endif
...
void DirectRenderer::decideRenderPassAllocationsForFrame(const RenderPassList& renderPassesInDrawOrder)
{
    ...
    RenderPassesHashMap renderPassesInFrame;
    ...
    for (passIterator = m_renderPassTextures.begin(); passIterator != m_renderPassTextures.end(); ++passIterator) {
        RenderPassesHashMap::const_iterator it = renderPassesInFrame.find(passIterator->first);
