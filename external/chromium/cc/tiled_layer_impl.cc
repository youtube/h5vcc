...
public:
    ...
    if (m_free_list.size() > 0) {
        DrawableTile* item = m_free_list.back();
        m_free_list.pop_back();
        return make_scoped_ptr(new (item) DrawableTile());
    }
#endif
    ...
    void operator delete(void* p) {
        m_free_list.push_back(reinterpret_cast<DrawableTile*>(p));
    }
#endif
...
#if defined(__LB_SHELL__)
    static std::vector<DrawableTile*> m_free_list;
#endif
...
#if defined(__LB_SHELL__)
std::vector<DrawableTile*> DrawableTile::m_free_list;
#endif
