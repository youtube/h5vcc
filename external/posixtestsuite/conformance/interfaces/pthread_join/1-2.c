...
void * threaded ( void * arg )
{
	...
	/* Now tell we're done */
	ret = gettimeofday((struct timeval *)arg, NULL);
...
int main( int argc, char *argv[] )
{
	...
	struct timeval ts_pre, ts_th, ts_post;
	...
	for ( sc = 0; sc < NSCENAR; sc++ )
	{
		...
		if ( ret == 0 )                        /* The new thread is running */
		{
			...
			if ( ( ts_th.tv_sec < ts_pre.tv_sec )
			        || ( ( ts_th.tv_sec == ts_pre.tv_sec ) && ( ts_th.tv_usec < ts_pre.tv_usec ) ) )
			{
				output( "Pre  : %d.%06d\n", ts_pre.tv_sec, ts_pre.tv_usec );
				output( "child: %d.%06d\n", ts_th.tv_sec, ts_th.tv_usec );
				output( "Post : %d.%06d\n", ts_post.tv_sec, ts_post.tv_usec );
				FAILED( "Child returned before its creation ???" );
			}

			if ( ( ts_post.tv_sec < ts_th.tv_sec )
			        || ( ( ts_post.tv_sec == ts_th.tv_sec ) && ( ts_post.tv_usec < ts_th.tv_usec ) ) )
			{
				output( "Pre  : %d.%06d\n", ts_pre.tv_sec, ts_pre.tv_usec );
				output( "child: %d.%06d\n", ts_th.tv_sec, ts_th.tv_usec );
				output( "Post : %d.%06d\n", ts_post.tv_sec, ts_post.tv_usec );
