...
struct __scenario
{
	...
	int sem;		/* This semaphore is used to signal the end of the detached threads execution */
...
#define CASE(det,expl,scp,spa,sco,sta,gua,ssi,desc,res) \
 { det, expl, scp, spa, sco, sta, gua, ssi, desc, NULL, res }
...
void scenar_init()
{
...
#if 0  // Stack size and page size need not be related for our feature set.
	...
	for (i=0; i<NSCENAR; i++)
	{
		...
		#endif
#endif  // Thread stack addresses, guards, and scheduler attributes not supported.
	...
	for (sc=0; sc < NSCENAR; sc++)
	{
		...
		if (ret == 0) /* The new thread is running */
		{
			...
			else
			{
				...
				do { usleep(200 * 1000); } while (scenarii[sc].sem == 0);
				--scenarii[sc].sem;
			}
		}
	}
